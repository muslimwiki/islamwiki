...
try {
    ...
    if ($result) {
        ...
        if (isset($settings['skin'])) {
            $temp_3fae0573 = htmlspecialchars($settings['skin']) . "</strong></p>";
            echo "<p class='success'>🎨 Stored skin: <strong>" . $temp_3fae0573;
        } else {
    ...
    echo "<p class='info'>🎨 SkinManager results:</p>";
        $temp_2037d0f9 = ($activeSkin ? get_class($activeSkin) : 'null') . "</p>";
        echo "<p><strong>Active skin object:</strong> " . $temp_2037d0f9;
        $temp_1d256de8 = htmlspecialchars($activeSkinName) . "</span></p>";
        echo "<p><strong>Active skin name:</strong> <span class='success'>" . $temp_1d256de8;

    if ($activeSkin) {
        $temp_2e350408 = htmlspecialchars($activeSkin->getName()) . "</p>";
        echo "<p><strong>Skin name:</strong> " . $temp_2e350408;
        $temp_75d6a8b8 = htmlspecialchars($activeSkin->getVersion()) . "</p>";
        echo "<p><strong>Skin version:</strong> " . $temp_75d6a8b8;
    }
    ...
    foreach ($availableSkins as $name => $skin) {
        ...
        $temp_c4e56a11 = htmlspecialchars($name) . "</strong>: " . htmlspecialchars($skin->getName()) . " (v" . htmlspecialchars($skin->getVersion()) . ")$status</p>";
        echo "<p>- <strong>" . $temp_c4e56a11;
    }
    ...
    echo "<p><strong>SettingsController would show:</strong></p>";
        $temp_b272e348 = htmlspecialchars($userActiveSkin) . "</span></p>";
        echo "<p>User active skin from database: <span class='success'>" . $temp_b272e348;
    ...
    foreach ($skinOptions as $name => $skin) {
        ...
        $temp_3ae71d2a = htmlspecialchars($name) . "</strong>$activeStatus</p>";
        echo "<p>- <strong>" . $temp_3ae71d2a;
    }

    echo "</div>";
} catch (\Throwable $e) {
        $temp_155a5fcc = htmlspecialchars($e->getMessage()) . "</p>";
        echo "<p class='error'>❌ Error: " . $temp_155a5fcc;
