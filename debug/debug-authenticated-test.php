...
try {
    ...
    echo "- Session Authenticated: " . ($session->isLoggedIn() ? 'Yes' : 'No') . "\n";
    
    // Test skin manager with authenticated user
    $skinManager = $container->get('skin.manager');
    $userSkin = $skinManager->getActiveSkinForUser(1);
    echo "- User 1 Active Skin: " . ($userSkin ? $userSkin->getName() : 'None') . "\n";
            ...
            foreach ($middleware as $mw) {
                if (get_class($mw) === 'IslamWiki\Http\Middleware\SkinMiddleware') {
                    ...
                    try {
                        ...
                        echo "- SkinMiddleware Execution Error: " . $e->getMessage() . "\n";
                    }
                    break;
                }
            }
