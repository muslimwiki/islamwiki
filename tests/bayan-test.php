...
try {
    ...
    $bayanFormatter = new \IslamWiki\Core\Formatter\BayanFormatter(
        $container->get('db'),
        $container->get(\Psr\Log\LoggerInterface::class)
    );

    // Get statistics
    $statistics = $bayanFormatter->getStatistics();
    $hubNodes = $bayanFormatter->getQueryManager()->getHubNodes(5);
    $recentNodes = $bayanFormatter->getNodeManager()->search('', [], 10);
        ...
        <div class="container mx-auto px-4 py-8">
            <div class="mb-8">
                ...
                <p class="text-lg text-gray-600">
                    Explore the interconnected world of Islamic knowledge
                </p>
            </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900">
                                <?= count($statistics['relationship_types'] ?? []) ?>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow">
                <div class="px-6 py-4 border-b border-gray-200">
                    ...
                    <p class="text-sm text-gray-600">
                        Recently added to the knowledge graph
                    </p>
                </div>
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition duration-200">
                            <div class="flex-1">
                                <h3 class="font-medium text-gray-900">
                                    <a href="/bayan/node/<?= $node['id'] ?>" class="hover:text-blue-600">
                                        <?= htmlspecialchars($node['title']) ?>
                                    </a>
                                </h3>
                                <p class="text-sm text-gray-600"><?= ucfirst($node['type']) ?></p>
                                <p class="text-xs text-gray-500 mt-1">
                                    <?= htmlspecialchars(substr($node['content'], 0, 150)) ?>
                                    <?= strlen($node['content']) > 150 ? '...' : '' ?>
                                </p>
                            </div>
                            <div class="ml-4 text-right">
                                <p class="text-xs text-gray-500">
                                    <?= date('M j, Y', strtotime($node['created_at'])) ?>
                                </p>
