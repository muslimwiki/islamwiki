 ...
 * This is the core security system that handles all authentication,
...
class Security
{
     ...
     */
    public function __construct(Session $session, Connection $db, Logger $logger, array $config = [])
    {
    ...
    {
        try {
            $this->logger->info("Security: Login attempt for username: $username");
            ...
            if (empty($user)) {
                $this->logger->warning("Security: User not found: $username");
                return false;
            }

            $userData = $user[0];
            $this->logger->info("Security: User found: ID={$userData['id']}, Username={$userData['username']}, IsAdmin={$userData['is_admin']}");

            // Verify password
            if (!password_verify($password, $userData['password'])) {
                $this->logger->warning("Security: Password verification failed for user: $username");
                return false;
            }

            $this->logger->info("Security: Password verified, logging in user");
            ...
            $this->logger->info("Security: Login successful for user: {$userData['username']}");
            return true;
        } catch (\Exception $e) {
            $this->logger->error("Security: Login error: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function register(array $userData): ?int
    {
        try {
            ...
            $this->logger->error("Security: Register error: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function user(): ?array
    {
        ...
        try {
            ...
            $this->logger->error("Security: User error: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function generatePasswordResetToken(string $email): ?string
    {
        try {
            ...
            $this->logger->error("Security: Generate password reset token error: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function resetPassword(string $token, string $newPassword): bool
    {
        try {
            ...
            $this->logger->error("Security: Reset password error: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function updateProfile(int $userId, array $data): bool
    {
        try {
            ...
            $this->logger->error("Security: Update profile error: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function changePassword(int $userId, string $currentPassword, string $newPassword): bool
    {
        try {
            ...
            $this->logger->error("Security: Change password error: " . $e->getMessage());
            return false;
        }
    }
}
