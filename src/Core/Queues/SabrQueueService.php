...
/**
 * Sabr Queue Service (صبر - Patience/Persistence)
 * 
...
class SabrQueueService
{
     ...
     */
    private function createDriver(array $config): SabrQueueDriverInterface
    {
        return match ($config['driver']) {
            'database' => new SabrDatabaseDriver($this->database, $config),
            'file' => new SabrFileDriver($config),
            'memory' => new SabrMemoryDriver($config),
     ...
     */
    public function pop(string $queue = 'default'): ?SabrJob
    {
        $driver = $this->getDriver();
     ...
     * Get queue driver
     */
    private function getDriver(): SabrQueueDriverInterface
    {
 ...
 */
interface SabrQueueDriverInterface
{
    ...
    public function pop(string $queue): ?SabrJob;
 ...
 */
class SabrDatabaseDriver implements SabrQueueDriverInterface
{
    private MizanDatabase $database;
 ...
 * File Queue Driver
 */
class SabrFileDriver implements SabrQueueDriverInterface
{
    private string $path;
    private array $stats = ['pushed' => 0, 'popped' => 0, 'failed' => 0];
    ...
    public function getFailedJobs(): array
    {
        ...
        foreach ($files as $file) {
            ...
            if ($jobData['attempts'] >= 3) {
                $failedJobs[] = new SabrJob($jobData);
            }
        }
 ...
 * Memory Queue Driver
 */
class SabrMemoryDriver implements SabrQueueDriverInterface
{
    ...
    public function pop(string $queue): ?SabrJob
    {
        ...
        foreach ($this->queues[$queue] as $key => $jobData) {
            if ($jobData['reserved_at'] === null && $jobData['available_at'] <= time()) {
                ...
                return new SabrJob($this->queues[$queue][$key]);
            }
        }
