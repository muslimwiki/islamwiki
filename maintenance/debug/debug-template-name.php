...
if (file_exists($templateFile)) {
    ...
    echo "Template file readable: " . (is_readable($templateFile) ? 'Yes' : 'No') . "\n";
}
...
try {
    ...
    foreach ($testNames as $name) {
        ...
        try {
            ...
            echo "  ✅ SUCCESS - Result length: " . strlen($result) . " characters\n";
            if (strlen($result) > 0) {
...
try {
    ...
    if ($loader instanceof \Twig\Loader\FilesystemLoader) {
        ...
        foreach ($paths as $path) {
            ...
            echo "    Settings dir exists: " . (is_dir($settingsDir) ? 'Yes' : 'No') . "\n";
            
            if (is_dir($settingsDir)) {
                $files = scandir($settingsDir);
                echo "    Settings files: " . implode(', ', array_filter($files, function($f) { return $f !== '.' && $f !== '..'; })) . "\n";
            }
        }
    }
